-- Fix the uuid = text error in comments_for_post_with_user function
...
BEGIN
    ...
    LEFT JOIN public.comment_votes cv ON c.id = cv.comment_id AND cv.user_id::TEXT = p_user_id
    LEFT JOIN public.user_profiles up ON c.user_id::TEXT = up.id::TEXT
